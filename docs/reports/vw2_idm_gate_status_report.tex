...
The table below is the committed post-fix offline summary recovered from the rescued Phase 1 run. It is not a fresh local regeneration under the current stricter cache/tokenizer guards because raw CALVIN frame/action files and a complete official tokenizer checkpoint are not available in this checkout.

\begin{center}
\begingroup
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{0.30\linewidth}rrrp{0.25\linewidth}}
\hline
...
These smoke runs validate plumbing on a synthetic mock environment. They do not prove real CALVIN closed-loop behavior. The repository also includes machine-readable mock-only rollout provenance in \texttt{results/phase0\_rollout\_metadata.json}.

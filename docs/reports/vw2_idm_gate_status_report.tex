...
\begin{itemize}
  ...
  \item Latent caches and window indexes now carry validation metadata and fail on stale specs, stale adapter identity, stale split identity, stale CALVIN frame provenance, duplicate cache keys, or key-set mismatch.
  ...
  \item Train and validation manifests are checked for shared episode IDs, shared file-backed episode sources, and overlapping CALVIN frame spans before dataset construction.
  ...
  \item Direct-policy, MLP, planner, verifier, teacher, resume, and CLI checkpoints are validated against explicit architecture and path guards before loading.
\end{itemize}
